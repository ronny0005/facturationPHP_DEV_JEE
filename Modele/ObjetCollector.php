...
SELECT DISTINCT 0 DL_NoIn,0 cbMarq,fArt.AR_Ref,fArt.AR_Design DL_Design,0 AG_No1,0 AG_No2,'' LS_NoSerie, 0 DL_Qte,
    ...
    public function addDocenteteEntreeProcess($do_piece, $do_date, $do_ref, $do_tiers, $ca_num, $longitude, $latitude)
    {
        include("Objet.php");
        include("DocEnteteClass.php");
        $result = $this->db->requete($this->getEnteteTable(2, 0, 0));
        $rows = $result->fetchAll(PDO::FETCH_OBJ);
        if ($rows != null) {
            $do_piece = $this->getEnteteDocument(2, 20, 0, "Entree");
            $this->db->requete($this->updateEnteteTable(2, 0, 0, $do_piece));
            ...
            $docEntete->DO_DateLivr = $do_date;
            $docEntete->type_fac = "Entree";
            $docEntete->setValueMvt();
            $docEntete->insert_docEntete();
            //           $this->db->requete($this->addDocenteteEntreeMagasin(20,$do_date, $do_ref, $do_tiers, $ca_num, $longitude, $latitude,$do_piece));
        }
    }

    public function addDocenteteEntreeInventaireProcess($do_date, $do_ref, $do_tiers, $ca_num, $longitude, $latitude)
    {
        $result = $this->db->requete($this->getLastPieceInv());
        $rows = $result->fetchAll(PDO::FETCH_OBJ);
        if ($rows != null) {
            $do_piece = $rows[0]->DO_Piece;
            $this->db->requete($this->addDocenteteEntreeMagasin(20, $do_date, $do_ref, $do_tiers, $ca_num, $longitude, $latitude, $do_piece));
        }
    }
    ...
    {
        $result = $this->db->requete($this->getLastPieceInv21());
        $rows = $result->fetchAll(PDO::FETCH_OBJ);
        if ($rows != null) {
            $do_piece = $rows[0]->DO_Piece;
            $this->db->requete($this->addDocenteteEntreeMagasin(21, $do_date, $do_ref, $do_tiers, $ca_num, $longitude, $latitude, $do_piece));
        }
    }

    public function getentete_document($do_domaine, $do_type, $do_souche)
    {
        return "exec CB_EqGreaterIDC_DOCUMENT $do_domaine,$do_type,$do_souche";
    }

    public function getDateEcheance($CT_Num, $date)
    {
        $date_ech = $date;
        $result = $this->db->requete($this->getClientByCTNum($CT_Num));
        $rows = $result->fetchAll(PDO::FETCH_OBJ);
        if ($rows != null) {
            ...
            $MR_No = $rows[0]->MR_No;
            if ($MR_No != 0) {
                $result = $this->db->requete($this->getOptionModeleReglementByMRNo($MR_No));
                $rows = $result->fetchAll(PDO::FETCH_OBJ);
                if ($rows != null) {
                    $nbjour = $rows[0]->ER_NbJour;
                    $jour = $rows[0]->ER_JourTb01;
                    $condition = $rows[0]->ER_Condition;
                    $result = $this->db->requete($this->calculDateEcheance($nbjour, $jour, $date_ech));
                    $rows = $result->fetchAll(PDO::FETCH_OBJ);
                    if ($rows != null) {
                        if ($condition == 0)
                            $date_ech = $rows[0]->JourNet;
                        if ($condition == 1)
                            $date_ech = $rows[0]->FinMoisCivil;
                        if ($condition == 2)
                            $date_ech = $rows[0]->FinMois;
                    }
                }
            }

        }
        $data = array('date_ech' => $date_ech);
    ...
    public function ResteARegler($do_piece, $do_domaine, $do_type, $avance)
    {
        return "SELECT CASE WHEN MTT_A_REGLER>=0 THEN CASE WHEN  MTT_A_REGLER>=$avance THEN 1 ELSE 0 END ELSE CASE WHEN  MTT_A_REGLER<=$avance THEN 1 ELSE 0 END END AS VAL
                FROM(SELECT ROUND(SUM(DL_MontantTTC) - ISNULL(SUM(RC_Montant),0),2) MTT_A_REGLER
                FROM (	SELECT DO_Domaine,DO_Type,DO_Piece,SUM(DL_MontantTTC)DL_MontantTTC 
                        FROM F_DOCLIGNE 
                        GROUP BY DO_Domaine,DO_Type,DO_Piece)D
                LEFT JOIN F_REGLECH R ON D.DO_Domaine=R.DO_Domaine AND D.DO_Type=R.DO_Type AND D.DO_Piece=R.DO_Piece
                WHERE D.DO_Piece='$do_piece' 
                AND D.DO_Domaine=$do_domaine 
                AND D.DO_Type=$do_type)A";
    }

    public function addDocligneTransfertDetailProcess($DO_Domaine, $DO_Type, $AR_Ref, $prix, $CT_Num, $DO_Piece, $DL_Qte, $MvtStock, $DE_No, $login)
    {
        $AR_PrixAch = "";
        $AR_Design = "";
        $AR_PrixVen = "";
        ...
        if ($rows != null) {
            ...
            if ($rows != null) {
                ...
                $result = $this->db->requete($this->insertDocligneDetailMagasin($DO_Domaine, $DO_Type, $CT_Num, $DO_Piece, $DO_Date, $AR_Ref, $AR_Design, $DL_Qte, $do_ref, $AR_PrixAch, $MvtStock, $U_Intitule, $DE_No, $montantHT, "", $login));
                $result = $this->db->requete($this->lastLigneByDOPieceTrsftDetail($DO_Piece));
                $rows = $result->fetchAll(PDO::FETCH_OBJ);
                return $rows;
            }
        }
    ...
    public function insertDocligneMagasin($ct_num, $do_piece, $do_date, $ar_ref, $ar_design, $dl_qte, $do_ref, $ar_prixach, $mvtstock, $u_intitule, $de_no, $montantht, $ca_num, $login, $DL_PUTTC, $machine)
    {
        $no_ref = 2;
        if ($mvtstock == 3)
            $no_ref = 1;
        ...
        $requete = "INSERT INTO [dbo].[F_DOCLIGNE]" .
            ...
            "VALUES" .
            "    (/*DO_Domaine*/2,/*DO_Type*/23,/*CT_Num*/'" . $ct_num . "',/*DO_Piece*/'" . $do_piece . "'" .
            ...
            "    ,/*DL_PUBC*/0,/*DL_Taxe1*/0,/*DL_TypeTaux1*/0,/*DL_TypeTaxe1*/0,/*DL_NoRefDL_Taxe2*/0,/*DL_TypeTaux2*/0" .
            ...
            "    ,/*EU_Enumere*/'" . $u_intitule . "',/*EU_Qte*/" . $dl_qte . ",/*DL_TTC*/0,/*DE_No*/'" . $de_no . "',/*cbDE_No*/'" . $de_no . "',/*DL_NoRef*/" . $no_ref .
    ...
    public function insertDocligneDetailMagasin($do_domaine, $do_type, $ct_num, $do_piece, $do_date, $ar_ref, $ar_design, $dl_qte, $do_ref, $ar_prixach, $mvtstock, $u_intitule, $de_no, $montantht, $ca_num, $login)
    {
        $no_ref = 2;
        if ($mvtstock == 3)
            $no_ref = 1;
        ...
        $requete = "INSERT INTO [dbo].[F_DOCLIGNE]" .
            ...
            "    ,/*DL_PieceBC*/'',/*DL_PieceBL*/'',/*DO_Date*/'" . $do_date . "',/*DL_DateBC*/'1900-01-01'" .
            "    ,/*DL_DateBL*/'" . $do_date . "',/*DL_Ligne*/ (SELECT (1+COUNT(*))*10000 FROM F_DOCLIGNE WHERE DO_Domaine=$do_domaine AND DO_Type=$do_type AND DO_PIECE='" . $do_piece . "'),/*DO_Ref*/'" . $do_ref . "',/*DL_TNomencl*/0" .
            "    ,/*DL_TRemPied*/0,/*DL_TRemExep*/0,/*AR_Ref*/'" . $ar_ref . "',/*DL_Design*/'" . $ar_design . "'" .
            "   ,/*DL_Qte*/" . $dl_qte . ",/*DL_QteBC*/" . $dl_qte . ",/*DL_QteBL*/0,/*DL_PoidsNet*/0" .
            "    ,/*DL_PoidsBrut*/0,/*DL_Remise01REM_Valeur*/0" .
            "    ,/*DL_Remise01REM_Type*/0,/*DL_Remise02REM_Valeur*/0" .
            "    ,/*DL_Remise02REM_Type*/0,/*DL_Remise03REM_Valeur*/0" .
            "   ,/*DL_Remise03REM_Type*/0,/*DL_PrixUnitaire*/0 " .
            "    ,/*DL_PUBC*/0,/*DL_Taxe1*/0,/*DL_TypeTaux1*/0,/*DL_TypeTaxe1*/0,/*DL_NoRefDL_Taxe2*/0,/*DL_TypeTaux2*/0" .
            "    ,/*DL_TypeTaxe2*/0,/*CO_No*/0,/*cbCO_No*/NULL,/*AG_No1*/0" .
            "    ,/*AG_No2*/0,/*DL_PrixRU*/" . $ar_prixach . ",/*DL_CMUP*/" . $ar_prixach . ",/*DL_MvtStock*/'" . $mvtstock . "'" .
            "    ,/*DT_No*/0,/*cbDT_No*/NULL,/*AF_RefFourniss*/''" .
            "    ,/*EU_Enumere*/'" . $u_intitule . "',/*EU_Qte*/" . $dl_qte . ",/*DL_TTC*/0,/*DE_No*/'" . $de_no . "',/*cbDE_No*/'" . $de_no . "',/*DL_NoRef*/" . $no_ref .
            "    ,/*DL_TypePL*/0,/*DL_PUDevise*/0" .
            "    ,/*DL_PUTTC*/0,/*DL_No*/ISNULL((SELECT MAX(DL_No)+1 FROM F_DOCLIGNE),0),/*DO_DateLivr*/'1900-01-01',/*CA_Num*/'" . $ca_num . "'" .
            "    ,/*DL_Taxe3*/0,/*DL_TypeTaux3*/0,/*DL_TypeTaxe3*/0," .
            "   /*DL_Frais*/0,/*DL_Valorise*/1,/*AR_RefCompose*/NULL" .
            "    ,/*DL_NonLivre*/0,/*AC_RefClient*/'',/*DL_MontantHT*/0,/*DL_MontantTTC*/0" .
            "    ,/*DL_FactPoids*/0,/*DL_Escompte*/0,/*DL_PiecePL*/'',/*DL_DatePL*/'1900-01-01'" .
            "    ,/*DL_QtePL*/0,/*DL_NoColis*/'',/*DL_NoLink*/0,/*cbDL_NoLink*/NULL" .
            "    ,/*RP_Code*/NULL,/*DL_QteRessource*/0,/*DL_DateAvancement*/'1900-01-01',/*cbProt*/0" .
            "    ,/*cbCreateur*/'AND',/*cbModification*/GETDATE()" .
    ...
    public function recalculStock($cbMarq, $type_fac)
    {
        ...
        if ($type_fac == "Retour" || $type_fac == "Achat" || $type_fac == "AchatPreparationCommande" || $type_fac == "PreparationCommande")
            $this->db->requete($this->updateArtStock($DE_No, $AR_Ref, -$DL_Qte, -($CMUP * $DL_Qte)));
        else
            $this->db->requete($this->updateArtStock($DE_No, $AR_Ref, $DL_Qte, ($CMUP * $DL_Qte)));
    }
    ...
    public function addDocligneFactureProcess($dl_mvtStock, $DO_Piece, $AR_Ref, $DL_Qte, $remise, $type_remise,$cat_tarif,$cat_compta,$prix,$do_domaine,$do_type,$login,$type_fac,$machine,$cbMarqEntete,$mobile,$user,$entete_prev="")
    {
        ...
        if (strcmp($type_fac, "Retour") == 0)
            $DL_Qte = -$DL_Qte;
        $rows = $docligne->getPrixClientHT($AR_Ref, $cat_compta, $cat_tarif, $prix, $val_rem, $DL_Qte, $type_fourn);
        $montantHT = ROUND($rows->DL_MontantHT, 0);
        ...
        $DL_MontantTTC = ROUND($rows->DL_MontantTTC, 0);
        $puttc = $rows->DL_PUTTC;
        ...
        $docligne->DL_MvtStock = $dl_mvtStock;
        $docligne->DO_Domaine = $DO_Domaine;
        $docligne->DO_Type = $DO_Type;
        $docligne->CT_Num = $CT_Num;
        $docligne->DO_Piece = $DO_Piece;
        $docligne->DO_Date = $DO_Date;
        $docligne->DO_Ref = $DO_Ref;
        $docligne->AR_Ref = $AR_Ref;
        $docligne->DL_Design = $AR_Design;
        $docligne->DL_Qte = $DL_Qte;
        $docligne->DL_QteBC = $DL_QteBC;
        $docligne->DL_QteBL = $DL_QteBL;
        $docligne->EU_Qte = $EU_Qte;
        $docligne->DL_Remise01REM_Valeur = $remise;
        $docligne->DL_PrixUnitaire = $pu;
        $docligne->DL_Taxe1 = $taxe1;
        $docligne->DL_Taxe2 = $taxe2;
        $docligne->DL_Taxe3 = $taxe3;
        $docligne->CO_No = $CO_No;
        $docligne->DL_PrixRU = $AR_PrixAch;
        $docligne->EU_Enumere = $U_Intitule;
        $docligne->DE_No = $DE_No;
        $docligne->DL_PUTTC = $puttc;
        $docligne->CA_Num = $CA_Num;
        $docligne->DL_MontantHT = $montantHT;
        $docligne->DL_MontantTTC = $DL_MontantTTC;
        $docligne->DL_Remise01REM_Type = $type_remise;
        $docligne->DL_QtePL = $DL_QtePL;
        $docligne->DL_QteBL = $DL_QteBL;
        $docligne->DL_TypePL = $dl_typl;
        $docligne->DL_TTC = $typeHT;
        $docligne->DL_TypeTaux1 = $TypeTauxTaxe1;
        $docligne->DL_TypeTaux2 = $TypeTauxTaxe2;
        $docligne->DL_TypeTaux3 = $TypeTauxTaxe3;
        $docligne->DL_TypeTaxe1 = $TypeTaxe1;
        $docligne->DL_TypeTaxe2 = $TypeTaxe2;
        $docligne->DL_TypeTaxe3 = $TypeTaxe3;
        $docligne->DL_PieceBL = $entete_prev;
        $docligne->DL_DateBC = $do_dateBC;
        $docligne->DL_DateBL = $do_dateBL;
        $docligne->DL_CMUP = $DL_CMUP;
        $docligne ->DL_DatePL; $do_datePL;
        $docligne->MACHINEPC = $machine;
        $docligne->setuserName($user,$mobile);
        $docligne->insertDocligneMagasin();
        if (strcmp($type_fac, "PreparationCommande") != 0 && strcmp($type_fac, "Devis") != 0 && strcmp($type_fac, "Avoir") != 0) {
            $result = $this->db->requete($this->updateArtStock($DE_No, $AR_Ref, -$DL_Qte, -(ROUND($AR_PrixAch, 2) * $DL_Qte)));
        }
        if (strcmp($type_fac, "PreparationCommande") == 0) {
            $result = $this->db->requete($this->updateArtStockReel($DE_No, $AR_Ref, $DL_Qte));
        }

        $result = $this->db->requete($this->updateArtStock($DE_No, $AR_Ref, -$DL_Qte, -(rOUND($AR_PrixAch, 2) * $DL_Qte)));

        $this->commandeStock($DE_No, $AR_Ref, $AR_Design);
        $this->modifiePrix($AR_Ref, $AR_Design, $AR_PrixAch, $AR_PrixVen, $prix, $pxMin, $DO_Piece, $login);
        ...
        if (strcmp($type_fac, "PreparationCommande") == 0 || strcmp($type_fac, "AchatPreparationCommande") == 0)
            $result = $this->db->requete($this->updateDLColis($cat_compta, $rows[0]->cbMarq));
        $result = $this->db->requete($this->getLigneFactureDernierElement($DO_Piece));
        $rows = $result->fetchAll(PDO::FETCH_OBJ);
    ...
    {
        $requete = "SELECT DO_Tiers,D.CT_NumPayeur,CASE WHEN DO_Tiers IS NULL THEN D.CT_NumPayeur ELSE DO_Tiers END Tiers,A.DO_Domaine,A.DO_Type,A.DO_Piece,DL_MontantTTC,RG_Montant,RC_Montant,ISNULL(DR_Regle,0) DR_Regle
                    FROM F_DOCENTETE A
                    LEFT JOIN ( SELECT DO_Domaine,DO_Type,DO_Piece, SUM(DL_MontantTTC) DL_MontantTTC 
                                FROM F_DOCLIGNE
                                GROUP BY DO_Domaine,DO_Type,DO_Piece)B ON A.DO_Domaine=B.DO_Domaine AND A.DO_Type=B.DO_Type AND A.DO_Piece=B.DO_Piece
                    ...
                    FULL OUTER JOIN (	SELECT DO_Domaine,DO_Type,DO_Piece,MAX(DR_No)DR_No,MAX(CT_NumPayeur) CT_NumPayeur,MAX(RG_Montant)RG_Montant,SUM(RC_Montant) RC_Montant
                                        ...
                                        GROUP BY DO_Domaine,DO_Type,DO_Piece) D ON D.DO_Domaine = A.DO_Domaine AND D.DO_Type = A.DO_Type AND D.DO_Piece = A.DO_Piece AND C.DR_No = D.DR_No
...
";
    ...
    public function addCReglementFacture($DO_Piece, $montant, $impute,$rg_type,$mode_reglement,$caisse,$do_domaine,$do_type,$date_reglt,$lib_reglt,$date_ech) {
        include("Objet.php");
        include("ReglementClass.php");
        include("CaisseClass.php");
        $result = $this->db->requete($this->getEnteteByDOPiece($DO_Piece,$do_domaine,$do_type));
        $rows = $result->fetchAll(PDO::FETCH_OBJ);
        $DO_Date = $date_reglt;
        $CT_Num = $rows[0]->CT_NumPayeur;
        ...
        $DO_Domaine = $rows[0]->DO_Domaine;
        $DO_Type = $rows[0]->DO_Type;
        $result = $this->db->requete($this->getCaisseByCA_No($caisse));
        $rows = $result->fetchAll(PDO::FETCH_OBJ);
        if($rows==null){
            $co_nocaissier = 0;
            $ca_no = 0;
        ...
        $result = $this->db->requete($this->lastLigneCReglement());
        $rows = $result->fetchAll(PDO::FETCH_OBJ);
        $rg_no = $rows[0]->RG_No;
        $result = $this->db->requete($this->getDocReglByDO_Piece($DO_Piece,$do_domaine,$do_type));
        $rows = $result->fetchAll(PDO::FETCH_OBJ);
        $record = $rows;
        if(!isset($rows[0]->DR_No)){
            $result = $this->db->requete($this->addDocRegl($do_domaine, $do_type, $DO_Piece, 0, $mode_reglement,$date_reglt));
            $result = $this->db->requete($this->getDocReglByDO_Piece($DO_Piece,$do_domaine,$do_type));
            $rows = $result->fetchAll(PDO::FETCH_OBJ);
        }
        $dr_no = $rows[0]->DR_No;
        $result = $this->db->requete($this->montantRegle($DO_Piece,$DO_Domaine,$DO_Type));
        $rows = $result->fetchAll(PDO::FETCH_OBJ);
        $montantTTC = $rows[0]->montantRegle;
        $result = $this->db->requete($this->AvanceDoPiece($DO_Piece,$DO_Domaine,$DO_Type));
        $rows = $result->fetchAll(PDO::FETCH_OBJ);
        $montantTTC_regle = $rows[0]->avance_regle;
        $reste_a_regler = $montantTTC - $montantTTC_regle;
        $this->db->requete($this->updateDateRegle($dr_no,$date_ech));
        if(($reste_a_regler>=0 && $montant>$reste_a_regler) || ($reste_a_regler<0 && $montant<$reste_a_regler)){
            $result = $this->db->requete($this->addReglEch($rg_no, $dr_no, $DO_Domaine, $DO_Type, $DO_Piece, round($reste_a_regler,2)));
        }else{
            $result = $this->db->requete($this->addReglEch($rg_no, $dr_no, $DO_Domaine, $DO_Type, $DO_Piece, round($montant,2)));
            if($montant==$reste_a_regler){
                $this->db->requete($this->updateDrRegle($dr_no));
                $this->db->requete($this->updateImpute($rg_no));
            }
        }
        return $rg_no;
    }
    ...
    public function cumulStockDetail()
    {
        return "SELECT  AR_Design
                ...
                FROM (  SELECT	AR_Design
                                ,DE_Intitule
                                ,SUM(ISNULL(AS_QteSto,0))-SUM(ISNULL(AS_QteMini,0)) Alerte
                                ,SUM(ISNULL(AS_QteMaxi,0))-SUM(ISNULL(AS_QteSto,0)) QteACommander
                                ,SUM(ISNULL(AS_QteSto,0)) AS_QteSto
                                ,SUM(ISNULL(AS_QteMini,0)) AS_QteMini
                        FROM F_ARTSTOCK A
                        INNER JOIN F_ARTICLE B ON A.AR_Ref=B.AR_Ref
                        INNER JOIN F_DEPOT C ON A.DE_No=C.DE_No
                        WHERE AS_QteMini<>0
                        GROUP BY AR_Design,DE_Intitule)A
                WHERE   Alerte<0";
    }

    public function updateTransBLtoFacture($entete_facture,$entete_bl){
        return"UPDATE
                F_DOCREGL SET DO_Piece='$entete_bl'
    ...
    public function getCompteEntree() {
        return "SELECT P_CreditCaisse FROM P_PARAMETRECIAL";
    }
    public function getCompteSortie() {
        return "SELECT P_DebitCaisse FROM P_PARAMETRECIAL";
    }
